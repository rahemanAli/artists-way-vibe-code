...
struct NextActionsListView: View {
    @Query(filter: #Predicate<Item> { $0.stateRaw == "engaged" }, sort: \Item.modifiedAt, order: .reverse)
...
struct SlipBoxView: View {
    ...
    @Query(filter: #Predicate<Item> { $0.typeRaw != "task" && $0.stateRaw != "inbox" }, sort: \Item.modifiedAt, order: .reverse)
            ...
            Section(header: Text("Metadata")) {
                ...
                if item.type == .task {
                    ...
                    Button("Add to Calendar") {
                         if let date = item.deferDate {
                             Task {
                                 _ = await CalendarManager.shared.addEvent(title: item.content, date: date)
                             }
                         }
                    }
                }
